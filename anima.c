...
static int num_coms, num_force_vecs ;
...
double	force_vec_s[MAX_LINE][4][2] ;
double	force_vec_l[MAX_LINE][4][2] ;
...
/* motionデータを読み込む */
int ReadMotdata(void)
{
	FILE *fp;
	char f_name[64],buffer[20000],*fgets();	
	int i,j,k,t,n;	
	double	angle, x_coord, z_coord ;
	int	d ;
	double trx, temp, tempx, tempz ;
	int pelvis_num ;
	int num_objects ;
	...
	//	printf("sprintf(f_name,.motgc, model_name );は終わったよmain.c\n");	//debug用
		...
		if ( fgets( buffer, 10000, fp ) == NULL ) return( -2 ) ;
...
//		printf("sprintf(f_name,.motgc, model_name );は終わったよmain.c\n");	//debug用
	for ( t = 0 ; ; t ++ ) {
	    ...
	    if ( buffer[0] != '#' ) {
		  real_time[cnt] = atof( strtok( buffer, "," ) ) ;
		    for( i = 0 ; i < num_objects ; i++ ) {
		      x_coord = atof( strtok( NULL, "," ) ) ;
		      z_coord = atof( strtok( NULL, "," ) ) ;
		      angle = atof( strtok( NULL, "," ) ) ;

		      p_mkr[cnt][marker_line_s[i]][0] = x_coord + 0.5 * length[i] * cos( angle ) ;
		      p_mkr[cnt][marker_line_s[i]][1] = 0.0 ;
		      p_mkr[cnt][marker_line_s[i]][2] = z_coord + 0.5 * length[i] * sin( angle ) ;
		      p_mkr[cnt][marker_line_f[i]][0] = x_coord - 0.5 * length[i] * cos( angle ) ;
		      p_mkr[cnt][marker_line_f[i]][1] = 0.0 ;
		      p_mkr[cnt][marker_line_f[i]][2] = z_coord - 0.5 * length[i] * sin( angle ) ;
		    }
...
//			printf("%d\n", cnt ) ;
	...
	fclose(fp); 
	  ...
	  sprintf(f_name,"%s_comgrf.csv", model_name );
	  if ((fp=fopen(f_name,"rt"))==NULL) {
		printf("Cannnot open %s\n",f_name);
		return(-3);
	  }
	  ...
	  cnt = 0 ;
		...
		if ( fgets( buffer, 10000, fp ) == NULL ) return(-2) ;
		  ...
		  num_coms = atoi( strtok( buffer, "," ) ) ;
		  num_force_vecs = atoi( strtok( NULL, "," ) ) ;
		  printf( "num_coms = %d, num_force_vecs=%d\n", num_coms, num_force_vecs ) ;
	...
	for ( t = 0 ; ; t ++ ) {
	    ...
	    if ( buffer[0] != '#' ) {
		  real_time[cnt] = atof( strtok( buffer, "," ) ) ;
		      body_cog[cnt][0] = atof( strtok( NULL, "," ) ) ;
		      body_cog[cnt][1] = atof( strtok( NULL, "," ) ) ;
		    for( i = 0 ; i < num_coms ; i++ ) {
		      seg_cog[cnt][i][0] = atof( strtok( NULL, "," ) ) ;
		      seg_cog[cnt][i][1] = atof( strtok( NULL, "," ) ) ;
		    }
		    for( i = 0 ; i < num_force_vecs ; i++ ) {
		      force_vec_s[cnt][i][0] = atof( strtok( NULL, "," ) ) ;
		      force_vec_s[cnt][i][1] = atof( strtok( NULL, "," ) ) ;
		      force_vec_l[cnt][i][0] = atof( strtok( NULL, "," ) ) ;
		      force_vec_l[cnt][i][1] = atof( strtok( NULL, "," ) ) ;
		    }
...
//			printf("%s", buffer ) ;
	...
	for ( t = 0 ; ; t ++ ) {
	    ...
	    if ( buffer[0] != '#' ) {
		  iframe = atoi( strtok( buffer, "," ) ) ;
		  real_time[cnt] = atof( strtok( NULL, "," ) ) ;
		    for( i = 0 ; i < num_mkr ; i++ ) {
		      p_mkr[cnt][i][0] = atof( strtok( NULL, "," ) ) ;
		      p_mkr[cnt][i][2] = atof( strtok( NULL, "," ) ) ;
		      p_mkr[cnt][i][1] = atof( strtok( NULL, "," ) ) * -1.0 ;
		    }

	
//			printf("%d", cnt ) ;
//			printf(", %f", real_time[cnt] ) ;
//			printf(", %f,%f,%f\n", p_mkr[cnt][4][0], p_mkr[cnt][4][1], p_mkr[cnt][4][2] ) ;
			cnt ++;
	    } // buffer != "#"
	}
	fclose(fp); 
...
//	  } // [ZZ] > 0.00001
	...
	if( ( p_mkr[timer][3][XX] - p_mkr[timer][1][XX] ) * ( p_mkr[timer][3][XX] - p_mkr[timer][1][XX] )
			+( p_mkr[timer][3][YY] - p_mkr[timer][1][YY] ) * ( p_mkr[timer][3][YY] - p_mkr[timer][1][YY] )
			+ ( p_mkr[timer][3][ZZ] - p_mkr[timer][1][ZZ] ) * ( p_mkr[timer][3][ZZ] - p_mkr[timer][1][ZZ] ) > 0.0001 ) {
             glMaterialfv(GL_FRONT,GL_DIFFUSE,material_color2 ); // 色を決めるgreen
		drawCylinderShading( p_mkr[timer][3][XX],p_mkr[timer][3][YY],p_mkr[timer][3][ZZ],
		 p_mkr[timer][1][XX],p_mkr[timer][1][YY],p_mkr[timer][1][ZZ], 0.015, 0); // 円柱を書く命令
		drawCylinderShading( p_mkr[timer][1][XX],p_mkr[timer][1][YY],p_mkr[timer][1][ZZ],
		 p_mkr[timer][4][XX],p_mkr[timer][4][YY],p_mkr[timer][4][ZZ], 0.015, 0); // 円柱を書く命令
		drawCylinderShading( p_mkr[timer][4][XX],p_mkr[timer][4][YY],p_mkr[timer][4][ZZ],
		 p_mkr[timer][2][XX],p_mkr[timer][2][YY],p_mkr[timer][2][ZZ], 0.015, 0); // 円柱を書く命令
	}
	...
	glPushMatrix();
	  ...
	  char s_time[100] ;
	  sprintf( s_time, "%f", (double)timer / 240.0 ) ;
	  YsDrawUglyFont( s_time,1,0);  // center of 0/1 is centering index, 
	glPopMatrix();
...
// 			printf ("%d", timer ) ;			// debug用
			break;
		case '>':
			if ( timer >= cnt-10 )	timer = 0 ;
			if ( timer < cnt-10 )	timer+=10 ;
			glutPostRedisplay();
// 			printf ("%d", timer ) ;			// debug用
			break;
		case 'b':
...
int main (int argc,char *argv[])
{
	...
	if(argc <= 1) {
		printf("USAGE: animation.exe -fFileName -mIntervalTime(msec)\n\n");
		exit(-1);
	}
	for (i=1; i<argc; i++) {
		if (argv[i][0] == '-') {
